...
```{r}
modeldata=modeldata %>% mutate(childteen=factor(childteen, levels=c('Age 9', 'Age 15')))%>%droplevels()
...
smoking_tbl=modeldata %>% 
  dplyr::select(smkPreg_binary, m1g2_YesNoPreg, m1g3_YesNoPreg, cm1inpov, PostnatalMaternalSmokingAny,
         ...
         Leukocytes_saliva, SSnewbornCT_center,  cg05575921, childteen)%>%
...
smoking_tbl=smoking_tbl%>%as_gt()%>%
  tab_row_group(label='Maternal', rows = 1:10)%>%
  tab_row_group(label='Child', rows=11:23)
...
# Supplemental Table `r sup_tab`:  Bivariate associations between prenatal maternal smoking (no/yes) and additional DNA methylation summary measures  among a diverse sample of `r n_total` children in the Fragile Families and Child Wellbeing study
...
```{r, eval=as_supp, fig.width=14, fig.height=10}
...
# Supplemental Table `r sup_tab`: Associations between prenatal maternal smoking and DNA methylation summary measures in multivariable linear models stratified by age or in multivariable linear mixed effect models among `r n_total` US children
...
# Supplemental Table `r sup_tab`: Associations between prenatal maternal smoking and DNA methylation summary measures in multivariable models among `r n_total` US children by ancestry using linear mixed effect models
...
# Supplemental Figure `r sup_fig`: Associations between prenatal maternal smoking and polymethylation scores calculated using alternative coefficient sets among `n n_total` children in the Fragile Families and Child Wellbeing study 
...
```{r supplemental figure compare across pms coefficients} 
 ...
 ggplot(aes(x=estimate, xmin=conf.low, xmax=conf.high, color=factor(age, rev(levels(age))), y=outcome))+
    geom_point(position = position_dodge(width = 1))+
    geom_errorbarh(height=0.1, position = position_dodge(width = 1))+
    facet_grid(predictors~ancestry, scales='free_x')+
    geom_vline(xintercept=0, color='red', linetype="dashed", alpha=0.5)+
    theme_classic()+
...
```{r compare across pms print, eval=as_supp, fig.width=10}
compare_pms
```
...
auc_fig=cowplot::plot_grid(fig4a, fig4b, labels=c('A', 'B'), nrow=1, rel_widths=c(0.65, 0.35), align='h', axis='tb')

ggexport(auc_fig, filename=file.path(outdir, paste0('Figure', main_fig, '.pdf')), width=9)
main_fig=main_fig+1
```

```{r main figure 4 auc print, eval=stand_alone, fig.width=10}
auc_fig
```

# Supplemental Table `r sup_tab`: Area under the curve for models shown in Main Figure 4 for classifying prenatal maternal smoke exposure, with *P* value when comparing to base model only or to model using polymethylation score only 
...
roc_ss=roc_table %>% filter(modeltype=='Sustained smoking polymethylation score (newborns, cell-type controlled)')%>%mutate(roc_pms=roc)%>%dplyr::select(childteen, roc_pms)
...
rct1%>%kbl(col.names=c('Age', 'Model', 'AUC', 'P value vs base model', 'P value vs polymethylation score'))
```

# Supplemental Table `r sup_tab`: Area under the curve when using different coefficient sets to construct polymethylation scores for predicting prenatal maternal smoke exposure
...
rct2%>%kbl(col.names=c('Age', 'Model', 'AUC', 'P value vs base model', 'P value vs polymethylation score with cell-type control'))
```
