...
roc_ss=roc_table %>% filter(modeltype=='Base model+Sustained smoking polymethylation score (newborns, cell-type controlled)')%>%mutate(roc_pms=roc)%>%dplyr::select(childteen, roc_pms)
...
rct1=roc_table%>% 
  ...
  dplyr::select(childteen, modeltype, auc, `P value vs base model`, `P value vs polymethylation score`)
