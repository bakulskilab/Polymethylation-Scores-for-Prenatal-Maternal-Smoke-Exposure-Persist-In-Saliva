...
set_label(modeldata$pediatric)='Pediatric epigenetic clock (years)'
set_label(modeldata$grim)='GRIM epigenetic clock (years)'
...
n_QCloss<-paste0("Methylation data QC: ", 
                 ...
                 N_M_count("cut %in% c('contaminated', 'relative') & predicted_sex_outlier==F & sex_new==predicted_sex & probe_fail_10==F & childteen!='M'", 
          ' with contamination/mislabels & \n ', pdqc_all),
...
myFF=left_join(myFF, methyldata)%>%mutate(idnum=as.character(id))
...
exclusion_tbl=myFF %>% mutate(childteen=case_when(childteen=='C'~'Age 9', childteen=='T'~'Age 15'))%>%
    dplyr::select(smkPreg_binary, childteen, MethylData2, SSnewbornCT_center, cg05575921, pediatric, globalmethylation, Leukocytes_saliva, Epithelial.cells_saliva, ancestry, cm1bsex, city_binary,  m1g2_YesNoPreg, m1g3_YesNoPreg, cm1inpov, PostnatalMaternalSmokingAny,SmkAtVisitPastmonth )%>%
...
smoking_tbl=modeldata %>% 
  dplyr::select(smkPreg_binary, childteen, SSnewbornCT_center, cg05575921, pediatric, globalmethylation, Leukocytes_saliva, Epithelial.cells_saliva, ancestry, cm1bsex,  city_binary, m1g2_YesNoPreg, m1g3_YesNoPreg, cm1inpov, PostnatalMaternalSmokingAny,SmkAtVisitPastmonth )%>%
  mutate(smkPreg_binary=factor(smkPreg_binary, levels=c('No', 'Yes'), labels=c('Not exposed', 'Smoke exposed')), 
         cm1bsex=factor(cm1bsex, levels=c('1 Boy', '2 Girl'), labels=c('Boy', 'Girl')))%>%
    mutate(cm1bsex=set_label(cm1bsex, 'Child gender'), smkPreg_binary=set_label(smkPreg_binary, 'Prenatal maternal smoking'),
         childteen=set_label(childteen, 'Age at DNA methylation sample'))%>%
...
smoking_tbl_extendedMeth=modeldata %>% 
  dplyr::select(smkPreg_binary, globalmethylation, Island_mean, OpenSea_mean, Shelf_mean, Shore_mean, 
                pediatric, grim, DNAmPACKYRS, Epithelial.cells_saliva, Leukocytes_saliva,
                SSnewbornCT_center, SSnewborn_center, SSolder_center, anynewborn_center,
                matches('cg'),  childteen)%>%
    mutate(smkPreg_binary=factor(smkPreg_binary, levels=c('No', 'Yes'), labels=c('Not exposed', 'Smoke exposed')))%>%
  tbl_strata(strata=childteen, .tbl_fun=~.x %>%
               tbl_summary(by=smkPreg_binary, statistic=
                             list(all_continuous()~"{mean} ({sd})"))%>%
...
data_text=all_models%>%
  ...
  unnest(tidied)%>%
  filter(outcome=='Sustained smoking polymethylation score (newborns, cell-type controlled)' & term=='smkPreg_binaryYes'& predictors=='base model')%>%mutate(outcome_f=outcome)
...
forest_plot=all_models%>%
  ...
  scale_y_discrete('', limits=c('European ancestry', 'Hispanic ancestry', 'African ancestry', 'Unstratified'), labels=c('European genetic ancestry', 'Hispanic genetic ancestry', 'African genetic ancestry', 'Unstratified'))+scale_color_manual('', values=c('black', 'darkgrey'))
...
# Supplemental Table `r sup_tab`: Coefficients for multivariable models of prenatal maternal smoke exposure regressed on selected DNA methylation summary variables (outcomes) among children in the Fragile Families and Child Wellbeing study using surrogate variables

```{r supplemental table surrogate variable table}
...
```{r supplemental table age and mixed effect print, eval=as_supp}
regression_table_age
```

# Supplemental Table `r sup_tab`: Associations between prenatal maternal smoking and DNA methylation summary measures in multivariable models among `r n_total` US children by ancestry using linear mixed effect models. All models controlled for:  first two principal components of child genetic ancestry (recalculated within ancestry strata for stratified models), child sex, maternal income-to-poverty ratio at birth, immune cell proportion estimated from methylation data, child age

```{r supplemental table by ancestry using linear mixed effect models}
test=rbind(longitudinal_global_models %>% mutate(ancestry='Unstratified'),
           longitudinal_local_models%>%dplyr::select(-data))%>%
  filter(predictors=='base model')%>%
  filter(outcome %in% my_outcomes)%>%
...
```{r supplemental table by ancestry using lme print, eval=as_supp}
...
```{r supplemental figure compare across pms coefficients} 
 compare_pms=all_models%>%
    ...
    unnest(tidied)%>%
    filter(term=='smkPreg_binaryYes')%>%
    filter(str_detect(outcome, 'smoking'))%>%
  filter(ancestry=='Unstratified')%>%
 ggplot(aes(x=estimate, xmin=conf.low, xmax=conf.high, color=factor(age, rev(levels(age))), y=outcome))+
    geom_point(position = position_dodge(width = 1))+
    geom_errorbarh(height=0.1, position = position_dodge(width = 1))+
...
ggexport(compare_pms, filename=file.path(outdir, paste0('SupplementalFigure', sup_fig, '.pdf')), width=9)
sup_fig=sup_fig+1
...
fig4a= global_roc%>%
  ...
  mutate(modeltype=gsub("Sustained smoking polymethylation score \\(newborns, cell-type controlled\\)", "Polymethylation score for prenatal maternal smoke exposure", modeltype))%>%
  mutate(modeltype=factor(modeltype, levels=c('AHRR: cg05575921', 'Base model+AHRR: cg05575921', 'Global methylation', 'Base model+Global methylation',  'Polymethylation score for prenatal maternal smoke exposure', 'Base model+Polymethylation score for prenatal maternal smoke exposure')))%>%
...
roc_table=global_roc %>% 
  dplyr::select(childteen, methylation, modeltype, roc, auc, ci_auc_delong, ci_auc_boot)%>%
...
#confidence_interval_difference_pms|base is the confidence interval for the DIFFERENCE between the two aucs - if we took 100 samples of the difference between the aucs, 95 of them would be between those two numbers: avoids incorrect error propagation, since the squares of errors add, not the errors themselves (see an explanation here: https://towardsdatascience.com/why-overlapping-confidence-intervals-mean-nothing-about-statistical-significance-48360559900a)
rct1=roc_table%>% 
  filter(methylation %in% c('globalmethylation', 'cg05575921', 'SSnewbornCT_center_scale', ''))%>%
  left_join(roc_base)%>%
  mutate(roctest_base=map2(roc, roc_basemodel, ~roc.test(.x, .y)))%>%
  left_join(roc_ss)%>%
  mutate(roctest_ss=map2(roc, roc_pms, ~roc.test(.x, .y)))%>%
  mutate(auc=round(auc, 2))%>%
  mutate(`P value vs polymethylation score`=map_dbl(roctest_ss, ~round(.x$p.value, 4)))%>%
  mutate(`P value vs base model`=map_dbl(roctest_base, ~round(.x$p.value, 4)))%>%
  mutate(confidence_interval_difference_base=map_chr(roctest_base, ~paste(round(.x$conf.int[1], 2), round(.x$conf.int[2], 2), sep=', ')))%>%
  mutate(confidence_interval_difference_pms=map_chr(roctest_ss, ~paste(round(.x$conf.int[1], 2), round(.x$conf.int[2], 2), sep=', ')))%>%
  dplyr::select(childteen, modeltype, auc, `P value vs base model`, `P value vs polymethylation score`, confidence_interval_difference_base, confidence_interval_difference_pms)

sup_tab=sup_tab+1  
```

```{r supplemental table auc print, eval=as_supp}
rct1%>%kbl(col.names=c('Age', 'Model', 'AUC', 'P value vs base model', 'P value vs base model + polymethylation score'))
```

# Supplemental Table `r sup_tab`: Area under the curve when using different coefficient sets to construct polymethylation scores for predicting prenatal maternal smoke exposure. With *P* value when comparing to base model only or to model using polymethylation score + base model. Base model: Child sex, maternal income to poverty ratio, immune cell proportion, plate, 1st two principal components from PCA on child genetic data, child age.  

```{r supplemental table auc other coefficients} 
rct2=roc_table%>% 
    filter(str_detect(modeltype, 'smoking|No methylation'))%>%
    left_join(roc_base)%>%
    mutate(roctest_base=map2(roc, roc_basemodel, ~roc.test(.x, .y)))%>%
    left_join(roc_ss)%>%
    mutate(roctest_ss=map2(roc, roc_pms, ~roc.test(.x, .y)))%>%
    mutate(auc=round(auc, 4))%>%
    mutate(`P value vs polymethylation score with cell-type control`=map_dbl(roctest_ss, ~round(.x$p.value, 4)))%>%
    mutate(`P value vs base model`=map_dbl(roctest_base, ~round(.x$p.value, 4)))%>%
    dplyr::select(childteen, modeltype, auc, `P value vs base model`, `P value vs polymethylation score with cell-type control`)
```

```{r supplemental table auc other coefficients print, eval=as_supp}
rct2%>%kbl(col.names=c('Age', 'Model', 'AUC', 'P value vs base model', 'P value vs base model + polymethylation score with cell-type control'))
```
